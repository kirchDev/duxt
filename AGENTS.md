...
`www/` is deployed as **one Worker with static assets**, built in GitHub Actions and uploaded with wrangler. There is no Pages git integration and no build on Cloudflare's side: `.github/workflows/deploy.yml` runs `pnpm deploy:www` on every push to `main` and again when a release is published, which builds with `NITRO_PRESET=cloudflare-module` and then `wrangler deploy`s the result. One place builds, and it is the one whose logs we keep.
