 ...
 * drifted apart. `apps/www/nuxt.config.ts` named three runtime paths and
 * `apps/www/wrangler.jsonc` named a different three; between them they left
...
function main(): void {
  ...
  try {
    ...
    throw new Error(
      'No build artifact at apps/www/.output. Run `pnpm --filter www build:cf` first — ' +
  ...
  if (!preset?.startsWith('cloudflare')) {
    throw new Error(
      `apps/www/.output was built with the \`${preset}\` preset. This check reads a Cloudflare ` +
