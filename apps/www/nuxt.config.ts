 ...
 *
 * The layer's own `package.json` in `packages/duxt` — the file release-please
 * bumps when it releases `@kirchdev/duxt`. Not the workspace root's, which is
 * private and carries no version at all.
...
const version = (() => {
  try {
    const pkg = JSON.parse(
      readFileSync(
        new URL('../../packages/duxt/package.json', import.meta.url),
        'utf8'
      )
    ) as { version?: string };
...
const adapterDatabase = () => {
  ...
  if (adapter === 'libsql') {
     ...
     * server was started from anywhere but `apps/www/`. The temp directory is the
...
export default defineNuxtConfig({
   ...
   *
   * In `apps/www/` and not in the layer: a consumer that extends duxt keeps the
