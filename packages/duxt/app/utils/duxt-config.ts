...
export const duxtDefaults: DuxtConfig = {
  ...
  navigation: [
    ...
    // `apps/www/app/app.config.ts` now.
   ...
   * sections at all. duxt's own sections live in `apps/www/app/app.config.ts`.
   */
  sections: [],
   ...
   * in `apps/www/app/app.config.ts`, where they are an example rather than a
   * default.
   *
  ...
  landing: {
    ...
    // through every site that extended it; they live in `apps/www/app/app.config.ts`
     ...
     * live in `apps/www/app/app.config.ts`, where each one POINTS at the page that
