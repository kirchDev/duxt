...
/** The workspace root, where the repository's `docs/` lives. */
const workspace = fileURLToPath(new URL('../../..', import.meta.url));

describe('documentation frontmatter', () => {
  const files = globSync('docs/**/*.md', { cwd: workspace });
  ...
  it.each(files)('%s parses as YAML', (file) => {
    expect(unreadableKeys(readFileSync(join(workspace, file), 'utf8'))).toEqual(
      []
    );
  });
});
