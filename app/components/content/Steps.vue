<template>
  <!-- Wraps an ordered list in Markdown into a stepped layout:
       ...
       clear of the rail they are supposed to sit on. `pl-0` on both puts the
       item's edge back at the container's own padding, which is what
       `-left-9` is measured against: 1.5rem in, 2.25rem back out, half a
       marker wide — centred on the line. -->
  <div
    class="my-6 border-l pl-6 [counter-reset:step] [&>ol]:ml-0 [&>ol]:list-none [&>ol]:pl-0 [&>ol>li]:relative [&>ol>li]:my-6 [&>ol>li]:pl-0 [&>ol>li]:[counter-increment:step] [&>ol>li]:before:absolute [&>ol>li]:before:-left-9 [&>ol>li]:before:flex [&>ol>li]:before:size-6 [&>ol>li]:before:items-center [&>ol>li]:before:justify-center [&>ol>li]:before:rounded-full [&>ol>li]:before:bg-muted [&>ol>li]:before:text-xs [&>ol>li]:before:font-medium [&>ol>li]:before:text-foreground [&>ol>li]:before:content-[counter(step)]"
  >
    <slot />
  </div>
</template>
