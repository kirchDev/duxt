...
## The Markdown is not localised

Only the interface is. `useDuxtPath()` strips the locale segment before every
content lookup, so every locale serves the same file — a site with seven locales
has seven versions of the interface and one set of pages. Content has no notion of a
locale, and the layer generates one collection per source and version, not one
per language. What that leaves as the route for translated documentation is in
