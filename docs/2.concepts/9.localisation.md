...
serves one tree in every locale — the interface changes language, the Markdown
does not — and that is the right shape for most documentation.
...
The active locale also supplies the document direction. Components use logical
start and end positions, so navigation, sheets, arrows and page furniture mirror
when a consumer adds an RTL locale. Technical material does not blindly mirror:
code, commands, paths, API names and other left-to-right syntax retain the
direction in which they can be read.

duxt ships no RTL locale of its own today. Direction-aware means a consumer can
add one without forking the theme; it is not a claim that one of the seven
built-in locales is RTL.
