...
| :----------------- | :------------------- | :-------------------------------------------------------- |
| `title`            | `'duxt'`             | O nome na barra de navegação e no título de SEO           |
| `version`          | `'v0.0.0'`           | O distintivo ao lado                                      |
...
| Chave        | Por omissão      | O que controla                                                |
| :----------- | :--------------- | :-------------------------------------------------------------- |
| `navigation` | duas entradas    | Ligações da barra; uma entrada com `children` vira um menu       |
...
| `landing`    | distintivo, título, descrição, uma ação, seis destaques | A página em `/`           |

Quatro são entregues vazios, por duas razões diferentes. `links`, `aside.links`
e `footer.legal` nomeiam um projeto concreto — um repositório, uma comunidade,
um aviso legal — e pertencem a quem gere o site. `sections` nomeia as partes de
...
| `headline`    | uma linha de texto       | O `h1`; recai sobre `title`                            |
| `description` | uma linha de texto       | O parágrafo por baixo, e a meta description da página  |
...
| `features`    | seis                     | A grelha de cartões                                    |
...
chamada; repeti-los sob seis cartões pede a um leitor acabado de chegar que
decida duas vezes.
