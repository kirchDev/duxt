...
| Prop       | Tipo   | Notas                                                  |
| :--------- | :----- | :------------------------------------------------------ |
| `releases` | array  | Cada una con `version`, `date`, `to`, `groups` y `contributors` |

El historial abre con **cuatro cifras** — versiones, cambios, colaboradores y el
día en que salió la última — dispuestas como una **rejilla de cuatro columnas a
todo el ancho**. La fecha es la
...
la página filtrada, con el total junto a la primera; al filtrar cambios
incompatibles, la cifra de colaboradores dice quién publicó alguno.

### De dónde salen los colaboradores

No del changelog, sino del rango git entre el tag anterior y esta versión. Se
prueban tanto `1.2.0` como `v1.2.0`. Los commits de merge marcan límites, pero no
reciben crédito como cambio propio; las cuentas marcadas como apps quedan fuera.
