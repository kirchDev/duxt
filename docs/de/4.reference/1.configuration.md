...
| `requestSamples`   | sieben von zwölf   | Welche Code-Samples der Try-it-Client anbietet — siehe [Request-Samples](/reference/openapi/request-samples) |
...
| :-- | :-- | :-- |
| `copy.models` | ChatGPT, Claude | Zielmodelle in Menüreihenfolge; die Liste ersetzt den Standard |
| `contributors.avatarUrl` | GitHub-Template | `{username}` wird je Contributor ersetzt |
| `toc.depth` | `3` | Tiefste Überschrift im Inhaltsverzeichnis |
| `toc.scrollOffset` | `96` | Aktivierungslinie unter dem Viewport-Rand |
| `search.fuzzy` | `0.35`, `3`, `20` | Fuse-Schwelle, Mindestlänge und Ergebnislimit |
| `search.recentPages` | `5` | Letzte Seiten; `0` blendet die Liste aus |
| `openapi.exampleDepth` | `6` | Maximale Tiefe abgeleiteter Beispiele |
| `openapi.schemaDepth` | `8` | Maximale Tiefe des Schema-Baums |
| `shortcuts.singleCharacter` | `true` | Globale `?`, `[` und `]`; `⌘/Strg+K` bleibt |
| `analytics.track` | **keine** | Provider-neutrales Callback je Leserinteraktion |
...
Listen ersetzen ihre Defaults. Eine leere `copy.models`-Liste entfernt externe
Modellziele, aber nicht Kopieren und Quellansicht. `search.recentPages: 0`
speichert keine neuen Einträge. Fuzzy-Optionen gelten nur für Beinahetreffer,
nachdem die exakte Content-Suche leer blieb. Analytics sendet ohne Callback
nichts; Einwilligung, Provider und Speicherung gehören der konsumierenden Site.
...
Wie du eine setzt, steht in [Deiner Website eine Marke geben](/guides/brand-your-site).
...
Ein seitenweiter Hinweis — etwa für ein Release, eine Migration oder ein
Wartungsfenster. Solange du keinen Eintrag schreibst, wird nichts gerendert und
kein Platz dafür freigehalten.

```ts [app/app.config.ts]
announcements: [
  {
    text: { 'en-GB': 'Version 2 is out.', 'de-DE': 'Version 2 ist da.' },
    link: { label: 'Release Notes lesen', to: '/releases/v2' }
  },
  {
    id: 'march-maintenance',
    text: 'Geplante Wartung am Sonntag ab 09:00 UTC.',
...
Die Liste wird in ihrer Reihenfolge gezeichnet: Release und Störung sind zwei
Ankündigungen, und das Ende der einen darf die andere nicht mit entfernen.

| Feld        | Typ                                               | Hinweise                                                         |
| :---------- | :------------------------------------------------ | :--------------------------------------------------------------- |
| `text`      | Text                                              | Satz als Literal, i18n-Key oder Locale-Record                   |
| `link`      | Link                                              | Eine Aktion; weitere gehören auf die verlinkte Seite            |
| `placement` | `above-header`, `below-header` oder `above-content` | Position; Standard ist `above-header`                           |
| `startsAt`  | `string`                                          | Beginn; alles, was `Date` parsen kann                            |
| `endsAt`    | `string`                                          | Ende                                                             |
| `id`        | `string`                                          | Identität fürs Schließen; sonst aus Inhalt und Link abgeleitet |

### Wo sie erscheinen

| Platzierung      | Ort                                                               |
| :--------------- | :---------------------------------------------------------------- |
| `above-header`   | Volle Breite am Browserrand über der Navigation — der Standard |
| `below-header`   | Volle Breite unter der Navigation, über Sidebars und Seite     |
| `above-content`  | In der Dokumentspalte bei Versions- und Übersetzungshinweisen   |

`above-content` gehört zur Dokumentspalte und erscheint deshalb auf
Dokumentationsseiten, nicht auf einer Landingpage ohne diese Spalte.

`announcementOptions.maxVisible` begrenzt, wie viele Hinweise eine Platzierung
gleichzeitig zeigt. Eine Zahl gilt für **jede** Platzierung, ein Objekt begrenzt
sie getrennt (`{ 'above-header': 1 }`); `null`, ein fehlender Schlüssel oder ein
fehlendes `maxVisible` bedeutet unbegrenzt. Die Grenze gilt für das Sichtbare:
Wird der oberste Hinweis geschlossen, rückt der nächste nach.

### Wann sie erscheinen

`startsAt` und `endsAt` werden im **Browser** gegen die Uhr des Lesers geprüft.
So beginnt und endet ein Hinweis ohne Deployment und ohne Neuladen der Seite.
Das Zeitfenster ist halboffen: sichtbar ab `startsAt` und bis `endsAt`.

Schreibe einen Offset, wenn der exakte Moment zählt
(`2026-03-01T09:00:00Z`). Ohne Zeitzone gilt die lokale Zeit des Lesers. Ein
ungültiges Datum wird wie ein fehlendes behandelt, damit ein Tippfehler den
Hinweis nicht unbemerkt verschluckt.

### Schließen

Ein geschlossener Hinweis bleibt über Navigation, Reload und Sprachwechsel
geschlossen. Gespeichert wird seine **Identität**:

- mit `id` genau diese locale-unabhängige Zeichenfolge; eine neue `id` zeigt den
  Hinweis allen erneut,
- ohne `id` ein Wert aus allen Locale-Varianten von `text` und `link`; neuer
  Wortlaut ist eine neue Ankündigung, eine andere Reihenfolge der Locale-Keys
  nicht.

Setze eine `id`, wenn du Tippfehler korrigieren willst, ohne den Hinweis erneut
anzuzeigen. Die Werte liegen nur im `localStorage` des Browsers; Einträge für
entfernte Ankündigungen werden verworfen. Weil Zeitfenster und Schließen dem
Browser gehören, wird der Streifen erst nach der Hydration gezeichnet.

Die Komponentenoberfläche steht bei
...
dem, der die Seite betreibt. `sections` benennt die Teile eines Dokumentations**baums**, und der
...
davon. Eine generierte Section unterhalb eines Bereichs (`/demo/openapi`) gehört zu
...
Heldenbereichs sind der Aufruf; sie unter einem Kartenraster zu wiederholen verlangt
...
[Release-Historie](/reference/changelog), deren Seiten je ein `date` tragen.
