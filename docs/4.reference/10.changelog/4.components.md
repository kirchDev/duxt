...
- the artefact came from a **downloaded source**, which Content clones at
  `--depth 1` — a history in which every commit looks as though one person wrote
  it is wrong data rather than missing data;
